...
export function PencocokanForm({
  ...
  // Calculate lines whenever connections change
  useEffect(() => {
    const calcLines = () => {
      ...
      Object.entries(data.jawaban).forEach(([kiriId, kananId], idx) => {
  ...
  }, [data.jawaban, data.itemKiri, data.itemKanan]);
        ...
        <div ref={containerRef} className="relative grid grid-cols-2 gap-4">
          ...
          {lines.length > 0 && (
            ...
            >
              <defs>
                {lines.map((line, i) => (
                  <linearGradient key={`grad-${i}`} id={`editMatchGrad-${i}`} x1="0%" y1="0%" x2="100%" y2="0%">

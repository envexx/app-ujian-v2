...
export default function EditUjianNewPage() {
  ...
  const handleAddSoal = async (tipe: TipeSoal) => {
    ...
    try {
      const response = await fetchApi(`/api/guru/ujian/${params.id}/soal`, {
        method: 'POST',
  ...
  const pendingUpdates = useRef<Record<string, Partial<Soal>>>({});
  ...
  const flushPendingUpdates = useCallback(async () => {
    ...
    const soalIds = Object.keys(updates);
    if (soalIds.length === 0) return;

    // Save all pending updates in parallel
    const promises = soalIds.map(async (soalId) => {
      try {
        const response = await fetchApi(`/api/guru/ujian/${params.id}/soal/${soalId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(updates[soalId]),
        });
        const result = await response.json();
        if (!result.success) {
          console.error(`Failed to update soal ${soalId}:`, result.error);
        }
      } catch (error) {
        console.error(`Error updating soal ${soalId}:`, error);
      }
    });

    await Promise.all(promises);
    console.log(`Batch saved ${soalIds.length} soal updates`);
  }, [params.id]);

  // Handle update soal — optimistic local update + batched API call (5 seconds)
  const handleUpdateSoal = useCallback((soalId: string, updates: Partial<Soal>) => {
    ...
    // 2. Merge updates into pending batch
    pendingUpdates.current[soalId] = {
      ...pendingUpdates.current[soalId],
      ...updates,
    };

    // 3. Reset batch timer (5 seconds debounce)
    if (batchTimer.current) {
      clearTimeout(batchTimer.current);
    }
    batchTimer.current = setTimeout(flushPendingUpdates, 5000);
  }, [flushPendingUpdates]);
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      try {
        const response = await fetchApi(`/api/guru/ujian/${params.id}/reorder-soal`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
  ...
  const handleSaveExamInfo = async () => {
    setIsSaving(true);
    try {
      const response = await fetchApi(`/api/guru/ujian/${params.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...examInfo,

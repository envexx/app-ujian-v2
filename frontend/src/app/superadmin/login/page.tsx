...
export default function SuperAdminLoginPage() {
  ...
  useEffect(() => {
    fetchApi("/api/superadmin/auth/session", { credentials: "include" })
      .then((res) => res.json())
      .then((data) => {
        if (data.success && data.isLoggedIn) {
          ...
          fetchApi("/api/auth/logout", { method: "POST", credentials: "include" }).catch(() => {});
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetchApi("/api/superadmin/auth/login", {
        method: "POST",

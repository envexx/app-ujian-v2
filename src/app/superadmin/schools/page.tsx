...
export default function SchoolsPage() {
  ...
  const handleEdit = async () => {
    ...
    setIsSubmitting(true);
    try {
      const res = await fetch(`/api/superadmin/schools/${editingSchoolId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(formData),
        credentials: "include",
  ...
  const handleToggle = async (schoolId: string) => {
    try {
      const res = await fetch(`/api/superadmin/schools/${schoolId}/toggle`, {
        method: "POST",
        credentials: "include",
      });
      const result = await res.json();
  ...
  const handleGenerateAdmin = async () => {
    ...
    setIsSubmitting(true);
    try {
      const res = await fetch(`/api/superadmin/schools/${selectedSchool.id}/generate-admin`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const openDetail = async (schoolId: string) => {
    try {
      const res = await fetch(`/api/superadmin/schools/${schoolId}`, { credentials: "include" });
      const result = await res.json();
      if (result.success) {
